...
def index():
    ...
    items = Item.query.with_entities(Item.id, Item.name, Item.description, Item.artist, Item.genre, Item.year, Item.designation, Item.image, Item.starting_value, Item.current_value, Item.bid_number, Item.status, Item.user_id, Item.date_posted, Watchlist.id.label("watchlist_id")).outerjoin(Watchlist).filter(or_(Watchlist.user_id==user_id, Watchlist.user_id == None)).all()
    ...
    popularitems = Item.query.with_entities(Item.id, Item.name, Item.description, Item.artist, Item.genre, Item.year, Item.designation, Item.image, Item.starting_value, Item.current_value, Item.bid_number, Item.status, Item.user_id, Item.date_posted, Watchlist.id.label("watchlist_id")).outerjoin(Watchlist).filter(or_(Watchlist.user_id==user_id, Watchlist.user_id == None)).order_by(Item.bid_number.desc()).limit(6)
